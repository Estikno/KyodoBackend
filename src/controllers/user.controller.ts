...
export async function updateUser(req: Request, res: Response): Promise<Response> {
    ...
    if(req.body.username){
        if(await User.findOne({username: req.body.username})) return res.status(400).json({message: 'Cant update username, it is repeated'});
    }

    if(req.body.email){
        if(await User.findOne({email: req.body.email})) return res.status(400).json({message: 'cannt update email, it is repeated'});
    }
    ...
    if(req.body.type === 'id'){
        ...
        try{
            user = await User.findOneAndUpdate({username: req.params.id}, req.body, {new: true});
        }
        catch (err) {
            return res.status(500).json({message: 'Internal server error, error: ' + err});
        }
